...
class Dashboard:
    ...
    def setup_sidebar(self):
        ...
        with col1:
            traffic_index = st.number_input(
                "Traffic Index", min_value=0.0, max_value=100.0, value=75.0
            )
            temperature = st.number_input(
                "Temperature (°C)", min_value=0.0, max_value=45.0, value=32.0
            )

        with col2:
            ndvi = st.slider(
                "Vegetation Health (NDVI)", min_value=0.0, max_value=1.0, value=0.45
            )
    ...
    def call_prediction_api(self, payload):
        ...
        try:
            response = requests.post(
                f"{API_BASE_URL}/api/v1/predict_net_impact", json=payload, timeout=30
            )
    ...
    def display_metrics(self, baseline_data, scenario_data):
        ...
        st.markdown(
            '<div class="main-header">🌏 Urban Net Pollution Policy Simulator</div>',
            unsafe_allow_html=True,
        )
        ...
        with col1:
            ...
            efficiency = abs(delta_co2) / (
                scenario_data.get("afforestation_increase_sqkm", 1) or 1
            )
        ...
        with col2:
            delta_pm25 = (
                scenario_data["net_pm25_tonnes_day"]
                - baseline_data["net_pm25_tonnes_day"]
            )
        ...
        with col3:
            delta_nox = (
                scenario_data["net_nox_tonnes_day"]
                - baseline_data["net_nox_tonnes_day"]
            )
            ...
            city_data.append(
                {
                    ...
                    "Primary Driver": np.random.choice(
                        ["High Forest Cover", "Low Traffic", "High NDVI"]
                    ),
                }
            )
    ...
    def display_policy_recommendations(self, scenario_params, results):
        ...
        if scenario_params["afforestation_increase"] > 0:
            efficiency = (
                abs(results.get("policy_impact_co2", 0))
                / scenario_params["afforestation_increase"]
            )
            if efficiency > 100:
                recommendations.append(
                    {
        ...
        if recommendations:
            for rec in recommendations:
                ...
                with st.expander(
                    f"{priority_color[rec['priority']]} {rec['policy']} - {rec['impact']} Impact"
                ):
                    st.write(rec["message"])
        else:
            st.info(
                "No strong policy recommendations based on current scenario. Try adjusting policy parameters."
            )
        ...
        if st.session_state.get("simulation_run", False):
            ...
            with col2:
                ...
                total_reduction = (
                    ...
                    + (
                        baseline["net_pm25_tonnes_day"]
                        - scenario["net_pm25_tonnes_day"]
                    )
                    * 1000
                    + (baseline["net_nox_tonnes_day"] - scenario["net_nox_tonnes_day"])
                    * 1000
                )
