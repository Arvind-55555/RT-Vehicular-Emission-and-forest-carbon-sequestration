    ...
    try:
        if os.path.exists(data_path):
            ...
            if "Net_CO2_kg" in df.columns:
                report_data["metrics"]["total_net_co2_kg"] = float(
                    df["Net_CO2_kg"].sum()
                )
                report_data["metrics"]["avg_net_co2_kg"] = float(
                    df["Net_CO2_kg"].mean()
                )

            if "Net_PM25_kg" in df.columns:
                report_data["metrics"]["total_net_pm25_kg"] = float(
                    df["Net_PM25_kg"].sum()
                )
            ...
            logger.warning(
                f"Data file not found at {data_path}. Generating empty report."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="Output directory for reports"
    )
