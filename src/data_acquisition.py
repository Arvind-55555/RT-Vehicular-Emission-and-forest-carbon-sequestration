...
import logging
from src.config import APIConfig, CITIES, FOREST_COVER_DATA, VEHICLE_DATA
...
class DataAcquisition:
    def __init__(self):
        self.gmaps_key = APIConfig.GOOGLE_MAPS_API_KEY
        self.weather_key = APIConfig.OPENWEATHER_API_KEY
    
    def get_traffic_data(self, city, routes=None):
        """
        ...
        try:
            ...
            base_traffic = {
                'Delhi': 75, 'Mumbai': 80, 'Bengaluru': 85, 
                'Chennai': 70, 'Kolkata': 65, 'Hyderabad': 72,
                'Pune': 68, 'Ahmedabad': 62, 'Surat': 60, 'Jaipur': 58
            }
            ...
            return {'traffic_index': 50, 'avg_speed_kph': 30, 'timestamp': datetime.now()}
    ...
    def get_weather_data(self, city):
        ...
        try:
            ...
            return {
                'max_temp_c': 30, 'humidity_pct': 60, 
                'wind_speed_ms': 3, 'pressure_hpa': 1013,
                'timestamp': datetime.now()
            }
    
    def get_ndvi_data(self, city_boundary_geojson):
        """
        ...
        try:
            ...
            return {'median_ndvi': 0.5, 'mean_ndvi': 0.48, 'ndvi_std': 0.05, 'timestamp': datetime.now()}
    ...
    def get_air_quality_data(self, city):
        ...
        try:
            ...
            return {
                'pm25_ug_m3': 150, 'nox_ug_m3': 80, 'aqi': 300,
                'timestamp': datetime.now()
            }
    
    def generate_training_data(self, start_date='2023-01-01', days=180):
        ...
        for i in range(days):
            ...
            for city in CITIES:
                ...
                for ward_id in range(1, 6):  # 5 wards per city
                    ...
                    ndvi_data = self.get_ndvi_data(None)  # Pass actual boundary in production
                    ...
                    record = {
                        ...
                        'total_vehicles': VEHICLE_DATA[city]['total_vehicles'] / total_wards,
        ...
        df = calculate_rolling_features(df, ['traffic_index_0_100', 'median_ndvi', 'max_temp_c'])
        
        logger.info(f"Generated training data with {len(df)} records")
        return df

def get_traffic_and_weather_data(cities=None, start_date='2023-01-01', days=30):
    """Main function to get comprehensive data."""
    if cities is None:
        cities = CITIES
    
    acquirer = DataAcquisition()
    return acquirer.generate_training_data(start_date, days)