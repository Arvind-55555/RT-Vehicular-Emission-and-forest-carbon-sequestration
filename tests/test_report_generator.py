...
class TestReportGenerator:
    ...
    def mock_data(self):
        return pd.DataFrame({
            'Net_CO2_kg': [100.0, 200.0],
            'Net_PM25_kg': [10.0, 20.0]
        })
    ...
    @patch('json.dump')
    def test_generate_report_success(self, mock_json_dump, mock_file, mock_read_csv, 
                                   mock_exists, mock_makedirs, mock_data):
        # Setup mocks
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('json.dump')
    def test_generate_report_missing_data(self, mock_json_dump, mock_file, 
